...
exports.sendMagicLink = async function(rootUrl, email){
  const user = await User.findOne({email: email});
  if(user){
    //reset the token and the expiration
    await user.setLoginToken();
    //the link will be an absolute link to the auth
    const link = `${rootUrl}/${user.loginToken}`;

    const body = await renderMarkdownTemplate("magic_link.md", {link: link});

    await sendEmail(`Login to ${settings.name}`, email, body);
  }else{
    //if we don't have this user, perhaps we could auto-register them here
    //depending on your site needs - other option is to register them and THEN
    //send the login link
  }
